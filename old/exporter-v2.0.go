...
func MySQLSlaveStatus() (map[string]string, error) {
    ...
    if err!= nil {
    	// вот здесь реализовать прочтение файла на List port /metrics
        return nil, "replication user dont connected with mysql db"
    }
    defer db.Close()
...
func main() {
    message := func(w http.ResponseWriter, r *http.Request) {
        ...
        // если подключиться неудалось, читаем форму из файла
        if err == "replication user dont connected with mysql db" {
        	scheme, err := os.Open("/etc/replication/status.scheme")
			scanner := bufio.NewScanner(scheme)
			for scanner.Scan() {
				responseText += fmt.Sprintf(scanner.Text())
			}
			fmt.Fprintf(w, responseText)
			

        }
        if err != nil && err != "replication user dont connected with mysql db" {
            fmt.Println(err)
        }
        ...
        responseText += fmt.Sprintf("mysql_Slave_SQL_Running{method=\"slave\"} %s\n", slaveSQLRunningState.Position)
